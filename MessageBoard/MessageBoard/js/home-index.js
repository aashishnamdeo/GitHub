...
var topicsController = ["$scope", "$http", "dataService", 
 function ($scope, $http, dataService) {
    ...
    if (dataService.isReady() == false) {
        $scope.isBusy = true;
        dataService.gettTopics()
            .then(function () {
                //success
            },
            function () {
                //error
                alert("Could not load the topic");
            })
            .then(function () {
                $scope.isBusy = false;
            });
...
function ($scope, $http, $window, dataService) {
    ...
    $scope.save = function () {
        dataService.addTopic($scope.newTopic)
            ...
            function () {
                alert("Could not able to Add newTopic")
            });
    };
}];
...
function ($scope, dataService, $window, $routeParams) {
    ...
    $scope.addReply = function () {
	    ...
	    function () {
                //error
                alert("cound not able to save reply")
            });
    };
}];